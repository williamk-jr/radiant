...
namespace Radiant {
	Texture::Texture(VulkanDevice&              device,
		...
		// GPU image.
		this->image = std::make_unique<VulkanImage>(memoryAllocator, VkExtent3D{width, height, 1}, VK_FORMAT_A8_UNORM,
		...
		commandBuffer.copyBufferToImage(stagingBuffer, *image, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, copyRegions);
