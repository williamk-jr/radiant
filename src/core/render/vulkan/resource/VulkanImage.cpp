...
namespace Radiant {
	VulkanImage::VulkanImage(VulkanMemoryAllocator& allocator, VkExtent3D extent)
	    : VulkanImage(allocator, extent, VK_FORMAT_B8G8R8A8_SRGB,
	                  VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT) {}

	VulkanImage::VulkanImage(VulkanMemoryAllocator& allocator, VkExtent3D extent, VkFormat format,
	...
	VulkanImage::VulkanImage(VulkanImage&& other) noexcept
	    : image(other.image), extent(other.extent), imageMemory(other.imageMemory),
