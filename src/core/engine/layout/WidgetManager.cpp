...
namespace Radiant {
	WidgetManager::WidgetManager(Window& window, StyleSheetParser::Parser& styleSheetParser)
	    : styleSheetParser(styleSheetParser) {
		Rect2D frameBufferSize = window.getFrameBufferSize();

		this->rootWidget = std::make_shared<Widget>(Widget{*this, frameBufferSize.width, frameBufferSize.height});
	}
	...
	RenderBatch WidgetManager::createRenderBatch() {
		...
		while (!widgets.empty()) {
			...
			StyleSheetParser::Color backgroundColor =
			    backgroundColorEntry.get<StyleSheetParser::ValueTypes::COLOR>(0).value();
