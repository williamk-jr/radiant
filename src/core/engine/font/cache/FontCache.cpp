...
namespace Radiant {
	FontCache::FontCache(size_t cacheSize, FontCacheType cacheType) {
		...
		if (error) {
			...
			Logger::info("Loaded Font Manager.", {{"FONT", MessageStyle::WHITE}}, 1);
		}
	...
	FT_Size FontCache::lookupPointFontSize(
	    FontCacheIdentifier fontFaceId, uint32_t width, uint32_t height, uint32_t xResolution, uint32_t yResolution) {
		FTC_ScalerRec scaler{};
		scaler.face_id = &fontFaceId;
	...
	FontCacheNode<FTC_SBit>
	FontCache::lookupBitmap(FontCacheIdentifier faceIdentifier, unsigned long charCode, int width, int height) {
		FTC_ImageTypeRec imageType{};
		imageType.face_id = &faceIdentifier;
		imageType.width   = width;
		imageType.height  = height;
	...
	FT_Error FontCache::requestFontFace(FTC_FaceID faceIdentifier,
	                                    FT_Library freetype,
