...
namespace Radiant {
	...
	class FontCache {
		public:
			...
			FT_Size lookupPointFontSize(FontCacheIdentifier fontFaceId,
			                            uint32_t            width,
			                            uint32_t            height,
			...
			lookupGlyph(FontCacheIdentifier faceIdentifier, unsigned long charCode, int width, int height);

			FontCacheNode<FTC_SBit>
			lookupBitmap(FontCacheIdentifier faceIdentifier, unsigned long charCode, int width, int height);
		...
		private:
			...
			static FT_Error
			requestFontFace(FTC_FaceID faceIdentifier, FT_Library freetype, FT_Pointer requestData, FT_Face* fontFace);
	};
} // namespace Radiant
